/* ESERCIZIO 1
  ...
  prima e gli ultimi 3 della seconda. Converti la stringa risultante in maiuscolo e 
  mostrala con un console.log().
*/

  const stringa1 = 'martina';
  const stringa2 = 'autore';
  const stringhe = function(stringa1,stringa2){
  let prima = stringa1.slice(0,2);
  let seconda = stringa2.slice(-3);
  let risultato = prima + seconda;
  risultato.toUpperCase ()
}
 stringhe()
...
*/
  const arr = list(range(1,99))
  const newarr = []
  const elements = function(){
  for (let i = 0; i < arr.length; i++) {
    newarr.push(i)
    const pari = newarr.filter(function(numero){
      return numero % 2 != 0
    })}
  }
  elements()
